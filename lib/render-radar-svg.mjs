...
// Turns a GitHub-style contribution calendar (array of weeks, each an array
// of 7 {date, count} day objects) into an animated "radar sweep" SVG.
//
// Visual concept: a scan-line sweeps left -> right across the grid on a
// loop, like an air-traffic-control radar. Cells with contributions "ping"
// (a soft ring pulse) exactly when the sweep crosses their column. Cells
// stay dim otherwise. Colour ramps from cool teal/cyan (low activity) to
// hot amber (peak activity), so the busiest days read as the "hottest"
...
/**
 * @param {Array<Array<{date:string,count:number}>>} weeks
 * @param {object} opts
 * @param {string} opts.title      label shown top-left, e.g. "CONTRIB://RADAR_SCAN"
 * @param {string} opts.subtitle   small readout top-right, e.g. "TOTAL 1,284 · STREAK 14D"
 * @param {number} opts.duration   seconds for one full sweep loop (default 22)
 */
...
<g stroke="${PALETTE.textMuted}" stroke-width="1.2" fill="none" opacity="0.7">
  ...
  const svg = `<svg width="${width}" height="${height}" viewBox="0 0 ${width} ${height}" xmlns="http://www.w3.org/2000/svg" font-family="'JetBrains Mono','Fira Code',ui-monospace,Menlo,Consolas,monospace">
