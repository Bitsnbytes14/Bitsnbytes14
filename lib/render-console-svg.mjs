...
import { roadBody } from './render-road-svg.mjs';
...
export function renderConsoleSVG(profile, opts = {}) {
  ...
  y += identity.height;
  const footerY = y + 8;
  const road = roadBody({ width });
  const roadY = footerY + footerH + 6;
  const totalHeight = roadY + road.height + 8;
  ...
  <text x="${width / 2}" y="${footerY + 18}" fill="${PALETTE.textMuted}" font-size="10.5" letter-spacing="1.4" text-anchor="middle">${esc(profile.footer || 'THE SUMMIT IS ALWAYS ONE MORE COMMIT AWAY')}</text>`;
  ...
  ${footerSvg}
  <g transform="translate(0,${roadY})"><svg x="0" y="0" width="${width}" height="${road.height}" viewBox="0 0 ${width} ${road.height}">${road.innerSvg}</svg></g>
  ${corners}
</svg>`;
}
