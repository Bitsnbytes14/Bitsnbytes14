...
export function renderConsoleSVG({ weeks, radarOpts = {}, profile, statsMetrics }) {
  ...
  const radarY = y;
  y += radar.height;
  const totalHeight = y + bottomPad;

  const spineX = 10;
  const spine = `<line x1="${spineX}" y1="${topPad + 4}" x2="${spineX}" y2="${totalHeight - bottomPad - 4}" stroke="${PALETTE.sweep}" stroke-width="1.2" opacity="0.35"/>`;
