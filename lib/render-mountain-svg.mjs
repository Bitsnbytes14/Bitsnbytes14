...
export function mountainBody(opts = {}) {
  ...
  // Climber: two alternating pose frames (reach → pull) that cross-fade,
  // which reads as climbing far better than a single pose being rocked.
  // Body is compact and vertical — knees bent UP and out, arms overhead,
  // nothing extended downward (extended legs read as running).
  // Drawn around origin so animateMotion carries it along the route.
  const CLIMB_DUR = 15;
  const CYCLE = 1.6;

  const limbStyle = `stroke="${PALETTE.accent}" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round" fill="none"`;

  // Frame A — extended reach: lead arm stretched high, trailing leg pushing
  const poseA = `
    <g ${limbStyle}>
      <circle cx="2" cy="-13" r="3.1" fill="${PALETTE.accent}" stroke="none"/>
      <path d="M1,-10 L-1,-2"/>
      <path d="M1,-10 L5,-16 L7,-23"/>
      <path d="M1,-10 L-5,-9 L-3,-15"/>
      <path d="M-1,-2 L5,-1 L4,6"/>
      <path d="M-1,-2 L-6,1 L-4,8"/>
    </g>`;

  // Frame B — pulled up and compact: body tucked, knees driven higher,
  // trailing arm now the one reaching
  const poseB = `
    <g ${limbStyle}>
      <circle cx="1" cy="-15" r="3.1" fill="${PALETTE.accent}" stroke="none"/>
      <path d="M0,-12 L-1,-4"/>
      <path d="M0,-12 L-4,-17 L-6,-24"/>
      <path d="M0,-12 L6,-13 L4,-19"/>
      <path d="M-1,-4 L5,-4 L5,3"/>
      <path d="M-1,-4 L-6,-2 L-5,5"/>
    </g>`;

  const climber = `
  <g>
    <g>
      <circle cx="0" cy="-6" r="16" fill="${PALETTE.accent}" opacity="0.10">
        <animate attributeName="opacity" values="0.05;0.20;0.05" dur="2.8s" repeatCount="indefinite"/>
      </circle>
      <g opacity="1">
        ${poseA}
        <animate attributeName="opacity" values="1;1;0;0;1" keyTimes="0;0.42;0.5;0.92;1" dur="${CYCLE}s" repeatCount="indefinite"/>
      </g>
      <g opacity="0">
        ${poseB}
        <animate attributeName="opacity" values="0;0;1;1;0" keyTimes="0;0.42;0.5;0.92;1" dur="${CYCLE}s" repeatCount="indefinite"/>
      </g>
      <!-- slight vertical bob synced to the pull, so the reach has weight -->
      <animateTransform attributeName="transform" type="translate"
        values="0,1.5; 0,1.5; 0,-1.5; 0,-1.5; 0,1.5" keyTimes="0;0.42;0.5;0.92;1"
