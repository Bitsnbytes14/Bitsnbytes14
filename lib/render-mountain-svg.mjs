...
export function mountainBody(opts = {}) {
  ...
  // Ascent beacon: an abstract tracked signal climbing the route, rather
  // than a figure. Reads cleanly at any size, matches the card's
  // instrument-panel language, and has no anatomy to get wrong.
  // A soft halo, a bright core, an expanding ping ring, and a short
  // trailing wake that suggests direction of travel.
  const CLIMB_DUR = 15;
  const climber = `
  <g>
    <g>
      <!-- trailing wake: short tapered streak behind the beacon -->
      <path d="M0,0 L-13,9" stroke="${PALETTE.accent}" stroke-width="2.6"
        stroke-linecap="round" opacity="0.28"/>
      <path d="M0,0 L-7,5" stroke="${PALETTE.accent}" stroke-width="3.2"
        stroke-linecap="round" opacity="0.5"/>

      <!-- outer halo, slow breathing -->
      <circle cx="0" cy="0" r="13" fill="${PALETTE.accent}" opacity="0.10">
        <animate attributeName="opacity" values="0.06;0.20;0.06"
          dur="2.6s" repeatCount="indefinite"/>
      </circle>

      <!-- expanding ping ring -->
      <circle cx="0" cy="0" r="4" fill="none" stroke="${PALETTE.accent}" stroke-width="1.3" opacity="0">
        <animate attributeName="r" values="4;17;17" dur="2.2s" repeatCount="indefinite"/>
        <animate attributeName="opacity" values="0.85;0;0" dur="2.2s" repeatCount="indefinite"/>
      </circle>

      <!-- bright core with a subtle pulse -->
      <circle cx="0" cy="0" r="4.2" fill="${PALETTE.accent}">
        <animate attributeName="r" values="3.8;4.8;3.8" dur="1.4s" repeatCount="indefinite"/>
      </circle>
      <circle cx="0" cy="0" r="1.8" fill="#fff6e6" opacity="0.9"/>
    </g>
